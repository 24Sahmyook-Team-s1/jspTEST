...
public class ProjectissueDAO {
    // 프로젝트 이슈 추가
    public void addIssue(int projectUserId, int projectId, String title, String description) throws NamingException, SQLException {
        String sql = "INSERT INTO projectissues (projectuserid, projectid, title, description, createdat) VALUES (?, ?, ?, ?, SYSDATE)";
        
        try (Connection conn = ConnectionPool.get(); 
             PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setInt(1, projectUserId); // 사용자 ID
    ...
    public List<ProjectissueObj> getIssuesByProjectId(int projectId) throws NamingException, SQLException {
        String sql = "SELECT projectissueid, projectuserid, title, description, createdat FROM projectissues WHERE projectid = ?";
